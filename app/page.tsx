...
function buildSchedule(selectedBooks, startDate, endDate, readingMode = "consecutive") {
  ...
  const chapters = readingMode === "random"
...
function getPlanStats(plan) {
  ...
  plan.assignments.forEach((day) => {
    const allDone = day.readings.length > 0 && day.readings.every((reading) => plan.completedChapterKeys.includes(reading.key));
...
function createPlanObject({ name, selectedBooks, startDate, endDate, color, readingMode = "consecutive", readingTime = "07:00", completedChapterKeys = [] }) {
  return {
...
function defaultEvents() {
  const today = todayISO();
  return [
    { id: crypto.randomUUID(), title: "Morning Prayer", type: "prayer", date: today, time: "06:30", notes: "Personal prayer and thanksgiving", remind: true, reminderMinutes: 10 },
    { id: crypto.randomUUID(), title: "Midday Prayer", type: "prayer", date: today, time: "12:00", notes: "Pause and pray", remind: true, reminderMinutes: 10 },
    { id: crypto.randomUUID(), title: "Wednesday Fast", type: "fast", date: addDays(today, 1), time: "06:00", notes: "Sunrise to sunset", remind: true, reminderMinutes: 30 },
    { id: crypto.randomUUID(), title: "Sunday Church", type: "church", date: addDays(today, 4), time: "10:00", notes: "Main service", remind: true, reminderMinutes: 60 },
    { id: crypto.randomUUID(), title: "Men’s Bible Study", type: "event", date: addDays(today, 6), time: "18:30", notes: "Church fellowship hall", remind: true, reminderMinutes: 30 },
  ];
}
...
function EventBadge({ type }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-xs capitalize ${styles[type] || styles.event}`}>
      {icons[type]}
      {type}
    </span>
  );
...
export default function BibleReadingPlannerApp() {
  ...
  const todaysPlans = useMemo(() => {
    const today = todayISO();
    return plans
      .map((plan) => ({ plan, todayReading: getTodaysReading(plan, today), stats: getPlanStats(plan) }))
      .filter((entry) => entry.todayReading && entry.todayReading.readings.length > 0);
  }, [plans]);

  const togglePreset = (preset) => {
    const presetBooks = PRESETS[preset] || [];
    setForm((prev) => ({ ...prev, preset, selectedBooks: presetBooks }));
  };
  ...
  const toggleBook = (bookName) => {
    setForm((prev) => {
      ...
      return {
        ...prev,
        preset: "custom",
        selectedBooks: exists ? prev.selectedBooks.filter((b) => b !== bookName) : [...prev.selectedBooks, bookName],
      };
    });
  };
  ...
  useEffect(() => {
    ...
    const checkReminders = () => {
      ...
      events.forEach((event) => {
        ...
        const key = `${event.id}-${event.date}-${event.time}-${event.reminderMinutes}`;
  ...
  const monthEventMap = useMemo(() => {
    ...
    return map;
  }, [upcomingEvents, plans]);

  const dayViewItems = useMemo(() => {
    const manualItems = upcomingEvents
    ...
    return [...manualItems, ...readingItems];
  }, [upcomingEvents, plans, selectedCalendarDate]);

  const todayFocusItems = useMemo(() => {
    const today = todayISO();
    const manual = upcomingEvents.filter((e) => e.date === today);
    const reading = plans.flatMap((plan) => {
      const assignment = plan.assignments.find((day) => day.date === today);
      if (!assignment || assignment.readings.length === 0) return [];
      return assignment.readings.map((reading) => ({
        ...reading,
        planId: plan.id,
        planName: plan.name,
        planColor: plan.color,
        done: plan.completedChapterKeys.includes(reading.key),
      }));
    });
  ...
  const createEvent = () => {
    ...
    setEventForm((prev) => ({ ...prev, title: "", notes: "", remind: true, reminderMinutes: 10 }));
  };
  ...
  const tabs = [
    ...
    { key: "build", label: "Build Plan", icon: Plus },
  ];
      ...
      <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
              ...
              <div className="max-w-3xl">
                ...
                <div>
                  <h1 className="text-3xl font-semibold tracking-tight text-white sm:text-5xl">
                    DiscipleOS — your daily operating system for Scripture, prayer, fasting, and church life.
                  </h1>
                  <div className="mt-4 text-lg italic text-[#D4A017]">
            ...
            <div className="space-y-4">
              <SectionCard className="p-5 sm:p-6">
                ...
                <div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
                  <div className="rounded-[24px] border border-white/10 bg-[linear-gradient(135deg,rgba(124,58,237,0.18),rgba(212,160,23,0.10))] border border-[#D4A017]/20 p-5">
                    <div className="mb-2 text-xs uppercase tracking-[0.18em] text-white/55">Verse of the day ✦</div>
                    <div className="text-xl font-semibold leading-8 text-white sm:text-2xl">
                      “{verseOfTheDay.text}”
                    </div>
              ...
              <SectionCard className="p-5 sm:p-6">
                ...
                <div className="space-y-3">
                  {todayFocusItems.manual.length === 0 ? (
                    <div className="rounded-[24px] border border-dashed border-white/10 p-5 text-sm text-white/60">
                      No spiritual events scheduled for today.
                    </div>
                    ...
                    <div key={event.id} className="rounded-[24px] border border-white/10 bg-white/[0.04] p-4">
                      ...
                      <div className="mt-1 text-sm text-white/55">{event.time} today</div>
                      ...
                      >
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            ...
                            <div className="mt-1 text-xs text-white/50">Reading time: {plan.readingTime || "07:00"}</div>
                          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
            <SectionCard className="p-5 sm:p-6">
              ...
              <div className="grid gap-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[{ key: "psalmsProverbs", label: "Psalms + Proverbs" }, { key: "newTestament", label: "New Testament" }, { key: "oldTestament", label: "Old Testament" }, { key: "wholeBible", label: "Whole Bible" }].map((preset) => (
                    <button
                      key={preset.key}
            ...
            <SectionCard className="p-5 sm:p-6">
              ...
              <div className="rounded-[24px] border border-white/10 bg-[linear-gradient(135deg,rgba(45,212,191,0.14),rgba(139,92,246,0.08))] p-5">
                <div className="mb-3 text-sm font-medium text-[#F8FAFC]">What this plan asks of the user</div>
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <div className="rounded-2xl border border-white/10 bg-black/10 p-4">
                    ...
                    <div className="mt-2 text-2xl font-semibold">
                      {planPreview.minPerDay === planPreview.maxPerDay ? `${planPreview.maxPerDay}` : `${planPreview.minPerDay}-${planPreview.maxPerDay}`}
                    </div>
                    <div className="text-xs text-white/50">chapters per day</div>
                  </div>
                </div>
                <div className="mt-4 rounded-2xl border border-white/10 bg-black/10 p-4 text-sm text-[#94A3B8]">
                  This plan will finish by <span className="font-medium text-white">{formatDate(form.endDate)}</span> if the assigned daily reading is completed.
                </div>
                <div className="mt-3 rounded-2xl border border-white/10 bg-black/10 p-4 text-sm text-[#94A3B8]">
                  Reading sessions will be placed on the calendar at <span className="font-medium text-white">{form.readingTime}</span>.
                </div>
              </div>
            </SectionCard>
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-[0.95fr_1.05fr]">
            <SectionCard className="p-5 sm:p-6">
              ...
              <div className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
                <div className="rounded-[24px] border border-white/10 bg-white/[0.04] p-4">
                  <div className="mb-3 flex items-center justify-between gap-3">
                    <div className="text-sm font-medium text-[#F8FAFC]">Add prayer, fast, church, Bible, or event</div>
                    <div className="inline-flex items-center gap-2 text-xs text-white/55">
                      <Smartphone className="h-3.5 w-3.5" />
                      Prompt scheduled activities
                    </div>
                  </div>
                  <div className="grid gap-3">
                    <input
                      ...
                      className="rounded-2xl border border-white/10 bg-slate-950/50 px-4 py-3 outline-none placeholder:text-white/35"
                    />
                    <div className="grid grid-cols-2 gap-3">
                      <select
                    ...
                    <div className="rounded-2xl border border-white/10 bg-[#0B1020]/50 p-3">
                      <div className="mb-3 text-sm text-[#F8FAFC]">Reminder</div>
                      <label className="flex items-center gap-3 text-sm text-white/70">
                        <input
                          type="checkbox"
                          checked={eventForm.remind}
                          onChange={(e) => setEventForm((prev) => ({ ...prev, remind: e.target.checked }))}
                          className="h-4 w-4"
                        />
                        Send a notification before this activity
                      </label>
                      {eventForm.remind ? (
                        <div className="mt-3 grid grid-cols-3 gap-2">
                          {[5, 10, 30].map((mins) => (
                            <button
                              key={mins}
                              onClick={() => setEventForm((prev) => ({ ...prev, reminderMinutes: mins }))}
                              className={cn(
                                "rounded-xl border px-3 py-2 text-sm transition",
                                Number(eventForm.reminderMinutes) === mins
                                  ? "border-white/30 bg-[#7C3AED] text-white"
                ...
                <div className="rounded-[24px] border border-white/10 bg-white/[0.04] p-4">
                  <div className="mb-4 flex items-center justify-between gap-3">
                    <div>
                      <div className="text-sm font-medium text-[#F8FAFC]">Calendar view</div>
                      <div className="text-sm text-white/55">Unified faith schedule with daily reading plans included</div>
                    </div>
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => setCalendarMonth((prev) => new Date(prev.getFullYear(), prev.getMonth() - 1, 1))}
                        className="rounded-xl border border-white/10 px-3 py-2 text-sm text-white/70 hover:bg-white/10 hover:text-white"
                      >
                        Prev
                      </button>
                      <div className="min-w-32 text-center text-sm font-medium text-[#F8FAFC]">{formatMonthLabel(calendarMonth)}</div>
                      <button
                        onClick={() => setCalendarMonth((prev) => new Date(prev.getFullYear(), prev.getMonth() + 1, 1))}
                        className="rounded-xl border border-white/10 px-3 py-2 text-sm text-white/70 hover:bg-white/10 hover:text-white"
                      >
                  ...
                  <div className="mb-4 grid grid-cols-7 gap-2 text-center text-xs uppercase tracking-wide text-white/40">
                  ...
                  <div className="overflow-x-auto pb-2">
                    ...
                    {monthDays.map((day) => {
                      ...
                      const eventsForDay = monthEventMap[iso] || [];
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            "min-h-36 rounded-2xl border p-3 text-left transition hover:bg-white/10",
                            selectedCalendarDate === iso
                              ? "border-violet-400/40 bg-violet-500/10"
                              : isToday
                                ? "border-white/30 bg-white/10"
                                : "border-white/10 bg-[#0B1020]/50",
                            isCurrentMonth ? "opacity-100" : "opacity-40"
                          )}
                        >
                          <div className="mb-2 flex items-center justify-between">
                            <div className="text-sm font-medium text-[#F8FAFC]">{day.getDate()}</div>
                            {eventsForDay.length > 0 ? (
                              <div className="text-[10px] text-white/45">{eventsForDay.length}</div>
                            ) : null}
                          </div>
                            ...
                            {eventsForDay.slice(0, 3).map((event) => (
                              ...
                              >
                                ...
                                <div className="truncate text-[10px] text-white/55">
                                  {event.isPlan ? event.notes : `${event.time} · ${event.type}`}
                                </div>
                              </div>
                            ))}
                            {eventsForDay.length > 3 ? (
                              <div className="text-[10px] text-white/45">+{eventsForDay.length - 3} more</div>
                            ) : null}
                          </div>
                        </button>
                      );
                ...
                <div className="rounded-[24px] border border-white/10 bg-white/[0.04] p-4">
                  <div className="mb-3 flex items-center justify-between gap-3">
                    <div>
                      <div className="text-sm font-medium text-[#F8FAFC]">Day view</div>
                      <div className="text-sm text-white/55">{formatDate(selectedCalendarDate)} at a glance</div>
                    </div>
                    <button
                  ...
                  <div className="max-h-80 space-y-3 overflow-auto pr-1">
                    {dayViewItems.length === 0 ? (
                      ...
                      <div key={event.id} className="rounded-2xl border border-white/10 bg-[#0B1020]/50 p-4">
                        {event.kind === "plan" ? (
                          <div>
                            <div className="mb-2"><EventBadge type="bible" /></div>
                            <div className="flex items-start justify-between gap-3">
                              <div>
                                <div className="text-base font-medium">{event.title}</div>
                                <div className="mt-1 text-sm text-white/60">{event.completedCount} / {event.readings.length} chapters complete</div>
                                <div className="mt-1 text-xs text-white/50">Scheduled for {event.readingTime || "07:00"}</div>
                                <div className="mt-2 flex flex-wrap gap-2">
                                  {event.readings.map((reading) => {
                                    const linkedPlan = plans.find((plan) => plan.id === event.planId);
                                    const done = linkedPlan?.completedChapterKeys.includes(reading.key);
                                    return (
                                      <button
                                        key={reading.key}
                              ...
                              <button
                                ...
                                className="rounded-xl border border-white/10 px-3 py-2 text-sm text-[#F8FAFC] hover:bg-white/10"
                              >
                                {event.completedCount === event.readings.length ? "Uncheck day" : "Mark day complete"}
                              </button>
                            </div>
                          </div>
                        ) : (
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              ...
                              <div className="text-base font-medium">{event.title}</div>
                              <div className="mt-1 text-sm text-white/60">{formatDate(event.date)} at {event.time}</div>
                              {event.notes ? <div className="mt-2 text-sm text-white/70">{event.notes}</div> : null}
                            </div>
                            <button
        ...
        {activeTab === "plans" && (
          <div className="grid gap-4 lg:grid-cols-[0.85fr_1.15fr]">
            <div className="space-y-4">
              <SectionCard className="p-5 sm:p-6">
                <div className="mb-3 text-lg font-semibold">Your rules</div>
                <div className="space-y-3">
                  {plans.map((plan) => {
                    const stats = getPlanStats(plan);
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            ...
                            <div className="mt-2 text-sm text-white/55">{formatDate(plan.startDate)} – {formatDate(plan.endDate)}</div>
                            <div className="mt-2 flex flex-wrap gap-2">
                              ...
                              <Pill>{plan.readingTime || "07:00"}</Pill>
            ...
            <SectionCard className="p-5 sm:p-6">
              {selectedPlan ? (
                <>
                  <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
                    <div>
                      ...
                      <h2 className="mt-3 text-2xl font-semibold">Rule breakdown</h2>
                      <p className="mt-1 text-sm text-white/60">{selectedPlan.selectedBooks.join(", ")}</p>
                      <div className="mt-3 flex flex-wrap gap-2">
                        <Pill>{selectedPlan.readingMode === "random" ? "Randomized reading order" : "Consecutive reading order"}</Pill>
                      </div>
                    </div>
                    {planStats && (
                  ...
                  {editingPlan && editingPlan.id === selectedPlan.id ? (
                    <div className="mb-5 rounded-[24px] border border-white/10 bg-white/[0.04] p-4">
                      ...
                      <label className="rounded-2xl border border-white/10 bg-slate-950/50 px-4 py-3 text-sm text-[#F8FAFC]">
                        <div className="mb-2">Reading time</div>
                        <input
                          type="time"
                      ...
                      {editPreview && (
                        <div className="mt-3 rounded-2xl border border-white/10 bg-[#0B1020]/50 p-4 text-sm text-white/70">
                          ...
                          <div className="grid gap-2 md:grid-cols-2">
                            ...
                            <div>Reading time: <span className="text-white">{editForm.readingTime}</span></div>
                          </div>
                        </div>
                      )}
                    ...
                    <>
                      <div className="mb-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                        ...
                        <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
                          ...
                          <div className="mt-1 text-xl font-semibold">{planStats.minPerDay === planStats.maxPerDay ? `${planStats.maxPerDay}` : `${planStats.minPerDay}-${planStats.maxPerDay}`}</div>
                          <div className="text-xs text-white/55">chapters per day</div>
                        </div>
                        <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
                      ...
                      <div className={cn(
                        ...
                        <div className="font-medium">{planStats.onTrack ? "On track to finish on time" : "Current pace is behind the target timeline"}</div>
                      ...
                      <div key={day.date} className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
                        <div className="mb-3 flex items-center justify-between gap-3">
                          <div>
                            ...
                            <div className="mt-1 text-xs text-white/50">Reading time: {selectedPlan.readingTime || "07:00"}</div>
                          ...
                          {day.readings.map((reading) => {
                            ...
                            return (
                              <button
                                ...
                                className={cn(
                                  "flex items-center justify-between rounded-2xl border px-3 py-3 text-left transition",
                                  done ? "border-emerald-400/30 bg-emerald-500/10" : "border-white/10 bg-[#0B1020]/50 hover:bg-white/5"
                                )}
                              >
                                <div>
                                  <div className="font-medium">{reading.book}</div>
                ...
                <div className="rounded-2xl border border-dashed border-white/10 p-6 text-white/60">Select a plan to view its schedule.</div>
              )}
            </SectionCard>
          </div>
        )}
